...
def build_agent():
    ...
    tools = [
        Tool.from_function(
            ...
            description="Calcula taxas de aumento, mortalidade, UTI e vacinação (gripe e COVID-19) " \
            "dos dados do OpenDataSUS processados.."
        ),
        Tool.from_function(
            func=charts_tool,
            name="Gerar Graficos",
            description="Gera dois graficos temporais de casos (ultimos 30 dias e ultimos 12 meses) " \
            "dos dados do OpenDataSUS processados."
        ),
        Tool.from_function(
            func=news_tool,
            name="Buscar Noticias",
            description="Busca noticias recentes sobre SRAG, gripe, COVID-19 e vacinacao no Brasil. " \
...
def run_analysis():
    ...
    query = (
        "Use suas ferramentas para coletar TODAS as métricas disponíveis (casos, mortalidade, UTI, vacina), "
        "gerar os gráficos de 30 dias e 12 meses, e buscar as 5 notícias mais relevantes. "
        "Sua Resposta Final deve ser apenas um compilado estruturado desses dados brutos, sem análise."
    )
